...
class StreamReassembler {
  private:
    ...
    ByteStream _output;  //!< The reassembled in-order byte stream 已经整理好了准备输出的流
    size_t _capacity;    //!< The maximum number of bytes
    std::vector<std::pair<char,bool>> _temp_stream; //一个循环缓冲区（类似于环形缓冲），存储乱序数据，并且每个位置用一个 pair 来表示是否有有效数据。
    size_t _cur_index;
    size_t _eof_index;
